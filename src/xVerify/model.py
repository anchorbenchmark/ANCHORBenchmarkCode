...
RETRY_TIMES = 50  # Maximum number of retry attempts
WAIT_TIME_UPPER = 120  # Upper bound for exponential backoff (seconds)
WAIT_TIME_LOWER = 10  # Lower bound for exponential backoff (seconds)
RATE_LIMIT_SLEEP = 60  # Extra sleep (seconds) on rate limit before tenacity retries
...
class Model:
    ...
    def __init__(
        ...
        self.top_p = top_p
        self._client = None  # lazily initialised once on first API call
    ...
    def _initialize_local_model(self) -> Tuple[AutoTokenizer, AutoModelForCausalLM]:
        ...
        if not os.path.exists(self.model_path_or_url):
            ...
            os.system(
                f'huggingface-cli download --resume-download <HF_ORG>/{self.model_name} --local-dir {self.model_path_or_url}')
    ...
    def _request_api(self, prompt: str) -> str:
        ...
        try:
            # Reuse the client across calls — creating a new OpenAI client on
            # every request adds TCP setup overhead for every single item.
            ...
            # Some newer models (like o1, o3, etc.) use max_completion_tokens instead of max_tokens
            # Also, some models (like Grok, o1, or newer Claude) don't allow both temperature and top_p
            # or require temperature to be exactly 1.
            params = {
            ...
            is_strict_model = any(m in self.model_name.lower() for m in ["o1-", "o3-", "gpt-5", "claude-haiku-4-5", "claude-4.5", "grok", "gemini"])
            ...
            # Handle temperature vs top_p conflict
            if is_strict_model:
                # Most "reasoning" or "strict" models either don't support temperature 
                # or require it to be 1.0. We'll omit them to use the provider's default.
                pass
            else:
                if self.temperature is not None:
                    params["temperature"] = self.temperature
                # Omit top_p to avoid conflict in strict models like Grok
            ...
            logger.warning(f"Rate limit hit — sleeping {RATE_LIMIT_SLEEP}s before retry: {repr(e)}")
            time.sleep(RATE_LIMIT_SLEEP)
            raise  # let tenacity retry after the sleep
        except Exception as e:
